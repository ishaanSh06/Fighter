...
function showWeaponInfo() {
  ...
  document.getElementById("maxBulls").value =
    viewport.target.weapon.gunInfo.maxBulls;
  document.getElementById("delay").value = viewport.target.weapon.gunInfo.delay;
  document.getElementById("reloadTime").value =
    viewport.target.weapon.gunInfo.reloadTime;
  document.getElementById("bullsPerTimes").value =
    viewport.target.weapon.gunInfo.bullsPerTimes;
  document.getElementById("hitRatio").value =
    viewport.target.weapon.gunInfo.hitRatio;
  document.getElementById("damage").value =
    viewport.target.weapon.bulletInfo.damage;
  document.getElementById("radius").value =
    viewport.target.weapon.bulletInfo.radius;
  document.getElementById("speed").value =
    viewport.target.weapon.bulletInfo.speed;
  document.getElementById("life").value =
    viewport.target.weapon.bulletInfo.life;
}

function applyCustomWeapons() {
  if (viewport.target) {
    viewport.target.weapon.gunInfo.maxBulls = Number(
      document.getElementById("maxBulls").value
    );
    viewport.target.weapon.gunInfo.delay = Number(
      document.getElementById("delay").value
    );
    viewport.target.weapon.gunInfo.reloadTime = Number(
      document.getElementById("reloadTime").value
    );
    viewport.target.weapon.gunInfo.bullsPerTimes = Number(
      document.getElementById("bullsPerTimes").value
    );
    viewport.target.weapon.gunInfo.hitRatio = Number(
      document.getElementById("hitRatio").value
    );
    viewport.target.weapon.bulletInfo.damage = Number(
      document.getElementById("damage").value
    );
    viewport.target.weapon.bulletInfo.radius = Number(
      document.getElementById("radius").value
    );
    viewport.target.weapon.bulletInfo.speed = Number(
      document.getElementById("speed").value
    );
    viewport.target.weapon.bulletInfo.life = Number(
      document.getElementById("life").value
    );
...
function loadText(url, success, fail) {
  ...
  xhr.onload = function () {
    ...
    if (status === 200) {
      if (success != null) success(xhr.response);
      return xhr.response;
    } else {
      if (fail != null) fail(status);
      return status;
    }
  };
  xhr.send();
...
function autoChat(sendMes, e, loop) {
  if (loop) {
    loadText(
      "https://api.minhhieu.asia/vi.php?text=" + sendMes,
      function (result) {
...
function addAICharacter() {
  ...
  for (var j = 1; j < floor(maxE / team + 1); j++) {
    ...
    for (var i = 0; i < team; i++) {
      var e = new AICharacter(
        null,
        pos.x + random(-dis, dis),
        pos.y + random(-dis, dis),
        null,
        null,
        j + 1
      );
  ...
  for (var i = 1; i < team; i++) {
    var e = new AICharacter(
      null,
      p.pos.x + random(-dis, dis),
      p.pos.y + random(-dis, dis),
      null,
      null,
      1
    );
...
window.onload = () => {
  ...
  document.getElementById("squad").addEventListener("click", (e) => {
    team = 4;
    clearChat();
    closeNav();
    start();
  ...
  document.getElementById("backToStartMenu").addEventListener("click", (e) => {
    runGame = false;
    showChat(false);
    menuWhenDie("close");
    openNav();
  ...
  setInterval(function () {
    if (runGame && focused) {
      if (iArr.length > world.maxItem) {
        for (var i = 0; i < iArr.length - world.maxItem; i++) iArr.shift();
      ...
      var nameGun = getValueAtIndex(
        weapons,
        floor(random(getObjectLength(weapons)))
      );
      iArr.push(
        new Item(random(gmap.size.x), random(gmap.size.y), null, null, nameGun)
      );
    }
  }, step * 1000);
}
...
function autoAddPlayers(step) {
  setInterval(function () {
    if (runGame && eArr.length < maxE) {
      var newCharacter = new AICharacter(
        null,
        random(gmap.size.x),
        random(gmap.size.y)
      );
...
function menuWhenDie(e, showWatch) {
  document.getElementById("menuWhenDie").style.display =
    e == "open" ? "block" : "none";
